...
- Autorisierung, persönliche Sonderrechte und Datenfilter verwenden die Zielperson. Alle Portal-POSTs sind gesperrt; nur das explizite Ende und Abmelden bleiben möglich. Mitteilungen werden nicht als gelesen markiert. Automatische Zuordnungen von Energiemesswerten und das Speichern abgelaufener Abstimmungen werden bei Support-Lesezugriffen unterdrückt; abgelaufene Abstimmungen erscheinen dennoch als beendet. Kalender-Abonnementlinks werden nicht ausgegeben, damit keine länger nutzbaren Zugangstoken für die Zielperson entstehen.
...
`scripts/qa-main-flows.sh` enthält zusätzlich `scripts/snapshot/qa-support-view.mjs`: Auswahl, Beginn und Ende sowie die permanente Kopfzeile einschließlich Fehlerseiten und langen Namen auf mehreren Seiten bei 320, 390, 768 und 1440 Pixel Breite. Das Skript akzeptiert ausschließlich lokale QA-Fixtures.
