...
| --- | --- | --- | --- | --- |
| 1 | Öffentlicher Einstieg und Anmeldung | Öffentlich, danach alle Rollen | Marketing-Host: `/` → „Pilot anfragen“ (`mailto:`). Tenant-Host: `/` → E-Mail-Formular `POST /auth/request` → Anmeldelink `GET /auth/verify` → `/app`. Alternative: OIDC-Start und Callback. Unterwege: vorbereiteter Link, erneut senden, ungültiger/abgelaufener/verbrauchter Link, Datenschutz, Abmelden und Browser-Zurück. | Die öffentliche Aktion öffnet die vorbereitete Anfrage beziehungsweise der private Hausüberblick wird sichtbar; nach Abmelden bleibt `/app` auch über Browser-Zurück geschützt. |
