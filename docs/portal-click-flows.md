...
| --- | --- | --- | --- | --- |
| 1 | Öffentlicher Einstieg und Anmeldung | Öffentlich, danach alle Rollen | Marketing-Host: `/` → HAUSV Home „starten“ → `/start` → Pfad und Eigentümer-E-Mail vormerken → Einmal-Link `/start/verify` → geschützter Connector-Vorbereitungsstand `/start/connector`. Tenant-Host: `/` → E-Mail-Formular `POST /auth/request` → Anmeldelink `GET /auth/verify` → `/app`. Alternative: OIDC-Start und Callback. Unterwege: erneut senden, ungültiger/abgelaufener/verbrauchter Link, Datenschutz, Abmelden und Browser-Zurück. | Der Zuhause-Pfad ist nach E-Mail-Bestätigung eindeutig reserviert, ohne bereits Home-Assistant-Geheimnisse anzunehmen; ein bestehendes Portal öffnet nach der Anmeldung den privaten Hausüberblick. |
