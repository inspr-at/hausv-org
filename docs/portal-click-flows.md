...
| --- | --- | --- | --- | --- |
| 1 | Öffentlicher Einstieg und Anmeldung | Öffentlich, danach alle Rollen | Marketing-Host: `/` → HAUSV Home „starten“ → `/start` → Pfad und Eigentümer-E-Mail vormerken → Einmal-Link `/start/verify` → geschützter Connector-Bereich `/start/connector` → Einmal-Code erzeugen → Linux-Connector laden und lokal koppeln. Unterwege: Code erneuern, Zugang rotieren oder widerrufen, erneut senden, ungültiger/abgelaufener/verbrauchter Link, Datenschutz, Abmelden und Browser-Zurück. Tenant-Host: `/` → E-Mail-Formular `POST /auth/request` → Anmeldelink `GET /auth/verify` → `/app`. Alternative: OIDC-Start und Callback. | Der Zuhause-Pfad ist eindeutig reserviert und der lokale Connector zeigt seinen aktuellen Lesestatus. Home-Assistant-Adresse und Token bleiben lokal; Messwertübertragung und Steuerung sind noch nicht aktiv. Ein bestehendes Portal öffnet nach der Anmeldung den privaten Hausüberblick. |
...
| 10 | Zuhause einrichten | Eigentümer, Verwalter, Admin; freigegebene technische Hilfe im erlaubten Umfang | `/app/zuhause/onboarding` → Verständnis → Zuhause-Art/Name → Verbraucher → Home Assistant → Messwerte bestätigen → Abschluss. Unterwege: Unterbrechung/Fortsetzen, keine Verbindung, unpassende Entitäten, Wohnung/Einfamilienhaus/Hausgemeinschaft. | Das Zuhause ist benannt, Messquellen sind verständlich zugeordnet und der Modus bleibt „Nur beobachten“. |
...
| 2, 7, 10, 11 | `qa-main-flows.mjs` | Hausüberblick, Anliegen in zwei Schritten samt Verlauf/No-JS, Energie-Onboarding mit Unterbrechung und mehrere Zuhause-Archetypen, Cockpit, Testlauf und Sofort-Rückkehr. |
