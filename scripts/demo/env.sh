...
WEG_USERS_JSON="$WEG_USERS_JSON,{\"email\":\"paul.sommer@musterstadt.example\",\"first_name\":\"Paul\",\"last_name\":\"Sommer\",\"phone\":\"+43 316 555 101\",\"role\":\"Verwalter\",\"status\":\"Aktiv\",\"tenants\":$demo_houses,\"auth_methods\":[\"email\"]}"

demo_add_user alina.auer@example.example Alina Auer Eigentümer janischhofweg-22 "+43 664 310 20 01"
demo_add_user matthias.dorn@example.example Matthias Dorn Mieter janischhofweg-22 "+43 676 310 20 02"
demo_add_user sibel.erdem@example.example Sibel Erdem Bewohner janischhofweg-22 "+43 660 310 20 03"
demo_add_user hedwig.eder@example.example Hedwig Eder Beirat janischhofweg-22 "+43 664 310 20 37"
...
INVITE_EMAILS='admin@example.com,vera.verwalter@musterstadt.example,paul.sommer@musterstadt.example,alina.auer@example.example,matthias.dorn@example.example,sibel.erdem@example.example,hedwig.eder@example.example,theresa.fink@example.example,gregor.haas@example.example,nora.illek@example.example,ivan.gruber@example.example,jasmin.kern@example.example,lorenz.leitner@example.example,mira.moser@example.example,marlene.karner@example.example,daniel.novak@example.example,petra.ortner@example.example,ramin.pichler@example.example,oskar.lind@example.example,elisa.rauch@example.example,simon.schober@example.example,leyla.tas@example.example,valentin.unger@example.example,carina.wolf@example.example,hannes.zeller@example.example,bettina.almer@example.example,emir.basic@example.example,klara.cerny@example.example,david.ebner@example.example,fatma.guel@example.example,josef.hofer@example.example,iris.jauk@example.example,kemal.kaya@example.example,lena.lenz@example.example,moritz.maier@example.example,nadine.oswald@example.example,peter.reiter@example.example,selma.sari@example.example,tobias.thaler@example.example,ulrike.weiss@example.example,florian.zechner@example.example,gerlinde.binder@example.example,armin.cakir@example.example'
