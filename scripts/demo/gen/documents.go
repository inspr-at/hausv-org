...
func buildDocuments(houses []house) []map[string]any {
	...
	for _, house := range houses {
		for _, kind := range kinds {
			...
			lines := []string{kind.title, house.Name, house.Address, "DEMO - synthetische Unterlage, kein rechtsgültiges Dokument", ""}
