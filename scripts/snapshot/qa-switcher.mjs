...
async function geometry(page,width,label) {
  ...
  const probe=await page.evaluate(() => {
    ...
    const pills=[...document.querySelectorAll('.context-bar .context-account > summary,.context-bar .context-scope summary')].filter(visible).map(e=>{
      ...
      const text=e.querySelector('.house-header-copy'),circle=e.querySelector('.disclosure-chevron');
  ...
  for(const pill of probe.pills){
    ...
    if(pill.scope){assert.equal(pill.paddingLeft,'12px',`${label}: scope left inset`);assert(pill.centered,`${label}: text and chevron vertically centred`);}
  }
...
async function checkRoute(page,width,path,label) {
  ...
  if (path.includes('/app/verwaltung')) {
    ...
    assert.equal(await visibleBar(page).locator('.context-scope .house-header-copy strong').textContent(), 'Alle Liegenschaften', `${label}: overview card title`);
    assert.match(await visibleBar(page).locator('.context-scope .house-header-copy small').textContent(), /^\d+ Liegenschaft(?:en)?$/, `${label}: overview card keeps property count`);
  ...
  if(width<=760){
    ...
    assert.equal(await menu.locator('[data-switcher]').count(),0,`${label}: no duplicate picker in drawer`);
    await menu.locator(':scope > summary').click();
  }else{
    assert.equal(await page.locator('aside.sidebar [data-switcher]').count(),0,`${label}: no duplicate picker in sidebar`);
  }
  checks.push(label);
}
...
async function checkPreview(width,role='Bewohner') {
  ...
  try {
    ...
    assert.equal((await bar.locator('.role-preview-trigger').textContent()).trim(),'Ansicht als …','normal view offers a role choice');
    ...
    assert.match(await bar.locator('[data-context-account] summary').innerText(),/Admin/,'preview retains real admin identity');
    assert.equal(await bar.evaluate(element=>getComputedStyle(element).backgroundColor),background,'preview retains shared light header');
    const picker=visibleBar(page).locator('.context-scope [data-switcher]');
    ...
    assert.match(await preview.innerText(),new RegExp(`Vorschau: ${role}-Sicht`),'active preview names the view');
    assert.match(await preview.innerText(),/Schreibgeschützt/,'preview names read-only status');
    assert.equal(await bar.locator('.role-preview-trigger').count(),0,'active preview does not offer another preview');
    await preview.getByRole('button',{name:'Ansicht verlassen',exact:true}).click();
    await bar.locator('.role-preview-trigger').waitFor({state:'attached'});
...
try {
  ...
  try{
    ...
    const picker=page.locator('[data-context-bar] .context-scope [data-switcher]');await picker.locator(':scope > summary').click();
    ...
    await page.locator('[data-context-bar] .context-scope [data-switcher] > summary').click();
    const back=page.locator('[data-context-bar] .context-scope [data-switcher] [data-switcher-initial] form').filter({has:page.locator('input[name="tenant"][value="demo"]')}).first();
  ...
  try{
    ...
    const picker=visibleBar(page).locator('.context-scope [data-switcher]');await picker.locator(':scope > summary').click();
