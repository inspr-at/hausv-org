...
func TestSupportViewChooserIsVisibleOnlyWithExplicitPermission(t *testing.T) {
	withPermission := newSupportViewTestApp(t, true)
	page := authedRequest(t, withPermission, "admin@example.com", "/demo/app/support-view")
	...
	withoutPermission := newSupportViewTestApp(t, false)
	page = authedRequest(t, withoutPermission, "admin@example.com", "/demo/app/support-view")
	if page.Code != http.StatusForbidden || strings.Contains(page.Body.String(), "data-support-view-start") {
...
func TestSupportViewIssueDetailKeepsBannerWithoutPublicLeak(t *testing.T) {
	a := newSupportViewTestApp(t, true)
	issue, err := issueRepositoryForTest(a, "demo").Create(residentIssue{
		TenantSlug:   "demo",
		AuthorEmail:  "resident@example.com",
	...
	if strings.Count(body, stylesheet) != 1 {
		t.Fatalf("legacy support stylesheet must appear exactly once")
	}
...
func TestSupportViewRejectsAllRegisteredAppWrites(t *testing.T) {
	a := newSupportViewTestApp(t, true)
	token, _ := startSupportViewForResident(t, a)
	...
	routes := regexp.MustCompile(`mux.HandleFunc\("POST (/app[^" ]*)"`).FindAllStringSubmatch(string(source), -1)
	if len(routes) < 50 {
		t.Fatalf("route inventory unexpectedly small: %d", len(routes))
	}
	for _, match := range routes {
		path := match[1]
		if path == "/app/support-view/end" {
			continue
		}
		path = regexp.MustCompile(`\{[^}]+\}`).ReplaceAllString(path, "probe")
		t.Run(path, func(t *testing.T) {
			if rr := supportViewPost(t, a, token, path, nil); rr.Code != http.StatusForbidden {
...
func TestSupportViewDoesNotExposeCalendarBearerCredentials(t *testing.T) {
	a := newSupportViewTestApp(t, true)
	token, _ := startSupportViewForResident(t, a)
	for _, path := range []string{"/app/settings", "/app/events", "/app/anliegen"} {
		rr := supportViewGet(t, a, token, path)
		if rr.Code != http.StatusOK || strings.Contains(rr.Body.String(), "/calendar/v1.") {
			t.Fatalf("%s exposed a reusable calendar credential or failed: %d", path, rr.Code)
		}
	}
}

func TestSupportViewRetainsOnlyTargetsExplicitCapabilityGrants(t *testing.T) {
	a := rightsTestApp(t)
	p := a.profiles["admin@example.com"]
	p.Permissions = []string{permissionSupportView}
	a.profiles[p.Email] = p
	for _, grant := range []store.UserCapabilityGrant{
		{Email: "resident@example.com", Capability: "manage-documents", Effect: "grant", UpdatedBy: "admin@example.com"},
		{Email: "admin@example.com", Capability: "manage-announcements", Effect: "grant", UpdatedBy: "admin@example.com"},
	} {
		if err := a.capabilityRepo("org").SetGrant(t.Context(), grant); err != nil {
			t.Fatal(err)
		}
	}
	token, _ := startSupportViewForResident(t, a)
	if rr := supportViewGet(t, a, token, "/app/dokumente/rechnungen/import"); rr.Code != http.StatusOK {
		t.Fatalf("target's document grant lost: %d", rr.Code)
	}
	if rr := supportViewGet(t, a, token, "/app/settings/users"); rr.Code != http.StatusForbidden {
		t.Fatalf("actor's user-management rights leaked: %d", rr.Code)
	}
	if err := a.capabilityRepo("org").SetGrant(t.Context(), store.UserCapabilityGrant{Email: "resident@example.com", Capability: "manage-documents", Effect: "deny", UpdatedBy: "admin@example.com"}); err != nil {
		t.Fatal(err)
	}
	if rr := supportViewGet(t, a, token, "/app/dokumente/rechnungen/import"); rr.Code != http.StatusForbidden {
		t.Fatalf("target's live deny ignored: %d", rr.Code)
	}
}

func TestSupportViewRestrictsMultiHouseTargetToSelectedMembership(t *testing.T) {
	a := newSupportViewTestApp(t, true)
	addTestTenant(a, tenantConfig{Slug: "haus-b", Name: "Private second house"})
	p := a.profiles["manager@example.com"]
	p.Tenants = []string{"demo", "haus-b"}
	a.profiles[p.Email] = p
	token, _ := startSupportViewForTarget(t, a, p.Email, roleManager)
	for _, path := range []string{"/app", "/app/liegenschaften", "/app/liegenschaften/suche?q=Private"} {
		rr := supportViewGet(t, a, token, path)
		if rr.Code != http.StatusOK || strings.Contains(rr.Body.String(), "Private second house") {
			t.Fatalf("%s crossed membership scope: %d", path, rr.Code)
		}
	}
	...
	a.handler().ServeHTTP(rr, req)
	if rr.Code != http.StatusSeeOther {
		t.Fatalf("foreign tenant status=%d", rr.Code)
	}
}

func TestSupportViewRefusesStartWithoutDurableAudit(t *testing.T) {
	a := newSupportViewTestApp(t, true)
	token, _, err := a.sessions.PutSession("admin@example.com", "demo", authMethodEmail, roleAdmin, time.Now().Add(time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	a.auditStore = nil
	rr := supportViewPost(t, a, token, "/app/support-view/start", url.Values{"tenant": {"demo"}, "target_email": {"resident@example.com"}, "target_role": {roleResident}})
	if rr.Code != http.StatusServiceUnavailable || len(rr.Result().Cookies()) != 0 {
		t.Fatalf("support started without audit: %d", rr.Code)
	}
	if _, ok := a.sessions.GetSession(token); !ok {
