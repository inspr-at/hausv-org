...
func TestMusterstadt2025ArchiveThroughRoutes(t *testing.T) {
	...
	if len(archived) != 26 || len(repos.documents.List()) != 39 {
	...
	response = archiveDemoRequest(t, a, archiveDemoManager, http.MethodPost, route, nil)
	if response.Code != http.StatusSeeOther || !reflect.DeepEqual(archived, annualStatementArchiveDocuments(repos.documents, run)) || len(repos.documents.List()) != 39 {
	...
	if next.Revision != 2 || response.Code != http.StatusSeeOther || len(annualStatementArchiveDocuments(repos.documents, next)) != 26 || len(repos.documents.List()) != 65 {
