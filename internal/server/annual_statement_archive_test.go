...
func TestMusterstadt2025ArchiveThroughRoutes(t *testing.T) {
	...
	if len(repos.documents.List()) != 11 {
		t.Fatal("seed must contain five original receipts and six portal documents")
	}
	...
	if !strings.Contains(beforePage.Body.String(), "Im Archiv ablegen") || len(repos.documents.List()) != 11 {
	...
	if len(archived) != 31 || len(repos.documents.List()) != 42 {
	...
	response = archiveDemoRequest(t, a, archiveDemoManager, http.MethodPost, route, nil)
	if response.Code != http.StatusSeeOther || !reflect.DeepEqual(archived, annualStatementArchiveDocuments(repos.documents, run)) || len(repos.documents.List()) != 42 {
	...
	if next.Revision != 2 || response.Code != http.StatusSeeOther || len(annualStatementArchiveDocuments(repos.documents, next)) != 31 || len(repos.documents.List()) != 73 {
