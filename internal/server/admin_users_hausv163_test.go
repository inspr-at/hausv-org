...
func TestAdoptOnEditGrantsConfigUserPermission(t *testing.T) {
	...
	a.profiles["resident@example.com"] = userProfile{Email: "resident@example.com", FirstName: "Res", LastName: "Ident", Role: roleRenter, Tenants: []string{"demo"}, AuthMethods: defaultAuthMethods()}

	if a.profileForTenant("resident@example.com", "demo").HasPermission(permissionParking) {
...
func TestPlainStoreRecordDoesNotEscalateConfigUser(t *testing.T) {
	...
	if _, err := a.inviteStore.Add(userProfile{Email: "resident@example.com", Role: roleAdmin, Tenants: []string{"demo"}, AuthMethods: defaultAuthMethods()}); err != nil {
		t.Fatalf("seed: %v", err)
	}
	if got := a.profileForTenant("resident@example.com", "demo").Role; normalizeRole(got) != roleResident {
...
func TestEditUserLoginMethods(t *testing.T) {
	...
	if a.isAuthMethodAllowed("invitee@example.com", "demo", authMethodEmail) {
		t.Fatal("email login should be disabled after restricting to OIDC")
	}
	if !a.isAuthMethodAllowed("invitee@example.com", "demo", authMethodOIDC) {
...
func TestEditUserEmptyLoginMethodsFallsBackToBoth(t *testing.T) {
	a := newTestPortalApp(t, userProfile{Email: "admin@example.com", Role: roleAdmin, Tenants: []string{"demo"}, AuthMethods: defaultAuthMethods()})
	if _, err := a.inviteStore.Add(userProfile{Email: "invitee@example.com", Role: roleResident, Tenants: []string{"demo"}, AuthMethods: []string{authMethodOIDC}}); err != nil {
		t.Fatalf("seed: %v", err)
	}
	res := authedFormRequest(t, a, "admin@example.com", "/demo/app/settings/users/edit", url.Values{
		"orig_email": {"invitee@example.com"},
...
func TestAdminCannotDemoteSelf(t *testing.T) {
	...
	if normalizeRole(a.roleFor("admin@example.com", "demo")) != roleAdmin {
...
func TestAdminCannotDeleteSelf(t *testing.T) {
	a := newTestPortalApp(t, userProfile{Email: "config-admin@example.com", Role: roleAdmin, Tenants: []string{"demo"}, AuthMethods: defaultAuthMethods()})
	if _, err := a.inviteStore.Add(userProfile{Email: "boss@example.com", Role: roleAdmin, Tenants: []string{"demo"}, AuthMethods: defaultAuthMethods()}); err != nil {
		t.Fatalf("seed: %v", err)
	}
	res := authedFormRequest(t, a, "boss@example.com", "/demo/app/settings/users/delete", url.Values{
...
func TestSecondAdminCanBeDeleted(t *testing.T) {
	...
	res := authedFormRequest(t, a, "config-admin@example.com", "/demo/app/settings/users/delete", url.Values{
		"email": {"boss@example.com"},
	})
...
func TestBreakGlassAdminProtectedAndAllowed(t *testing.T) {
	...
	a.profiles["breakglass@example.com"] = userProfile{Email: "breakglass@example.com", Role: roleAdmin, Status: "Aktiv", Tenants: []string{"demo"}, AuthMethods: defaultAuthMethods()}
	...
	if normalizeRole(a.roleFor("breakglass@example.com", "demo")) != roleAdmin {
		t.Fatal("break-glass admin must resolve to Admin")
	}

	// editing a break-glass admin is refused.
	res := authedFormRequest(t, a, "other-admin@example.com", "/demo/app/settings/users/edit", url.Values{
	...
	if _, err := a.inviteStore.Add(userProfile{Email: "breakglass@example.com", Role: roleResident, Adopted: true, Tenants: []string{"demo"}, AuthMethods: []string{authMethodOIDC}}); err != nil {
...
func TestNonAdminCannotManageUsers(t *testing.T) {
	...
	res := authedFormRequest(t, a, "resident@example.com", "/demo/app/settings/users/edit", url.Values{
		"orig_email": {"resident@example.com"},
		"email":      {"resident@example.com"},
		"role":       {roleAdmin},
	})
...
func TestManagerCannotEscalateToAdmin(t *testing.T) {
	...
	res := authedFormRequest(t, a, "manager@example.com", "/demo/app/settings/users/edit", url.Values{
		"orig_email":   {"resident@example.com"},
		"email":        {"resident@example.com"},
		"role":         {roleAdmin},
		"auth_methods": {"email"},
	...
	if normalizeRole(a.profileForTenant("resident@example.com", "demo").Role) == roleAdmin {
