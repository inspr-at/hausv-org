...
func TestMusterstadt2025DeliveryThroughRoutes(t *testing.T) {
	...
	if !strings.Contains(location, "sent=25") || !strings.Contains(location, "failed=0") || !strings.Contains(location, "skipped=0") {
		t.Fatal(location)
	}
	...
	for _, want := range []string{"25 gesendet · 0 fehlgeschlagen · 0 übersprungen", "Versandprotokoll", "Adresse", "Gesendet"} {
		if !strings.Contains(page.Body.String(), want) {
			t.Fatal("page missing", want)
		}
	}
	response = archiveDemoRequest(t, a, archiveDemoManager, http.MethodPost, route, nil)
	if response.Code != http.StatusSeeOther || !strings.Contains(response.Header().Get("Location"), "skipped=25") {
		t.Fatal(response.Code, response.Header())
	}
	entries, _ = os.ReadDir(directory)
	rows, _ = repos.annualStatementDeliveries.List(run.ID)
	if len(entries) != 25 || len(rows) != 25 {
	...
	if len(events) != 2 || events[0].Details["skipped"] != "25" || events[1].Details["sent"] != "25" {
		t.Fatal(events)
	}
	...
	t.Log("fresh SQLite seed → run → archive → send: 25 MIME files, each byte-identical to its party archive; 25 sent rows; retry sends zero")
}
...
func TestAnnualStatementDeliveryFailureRetryIntegrityAndCancellation(t *testing.T) {
	...
	if w.Code != http.StatusSeeOther || !strings.Contains(w.Header().Get("Location"), "failed=25") || failure.calls != 25 {
	...
	a.mailer = sink
	w = archiveDemoRequest(t, a, archiveDemoManager, http.MethodPost, route, nil)
	if w.Code != http.StatusSeeOther || !strings.Contains(w.Header().Get("Location"), "sent=24") || !strings.Contains(w.Header().Get("Location"), "failed=1") {
	...
	w = archiveDemoRequest(t, a, archiveDemoManager, http.MethodPost, route, nil)
	if !strings.Contains(w.Header().Get("Location"), "sent=1") || !strings.Contains(w.Header().Get("Location"), "skipped=24") {
		t.Fatal(w.Header())
	}
	entries, _ = os.ReadDir(directory)
	rows, _ = repos.annualStatementDeliveries.List(run.ID)
	if len(entries) != 25 || len(rows) != 51 {
