...
func (a *app) verwaltungSettingsAction(w http.ResponseWriter, r *http.Request, ac authCtx) {
	...
	for _, tenant := range a.managedTenants(&ac) {
		a.recordAudit(store.AuditEvent{TenantSlug: tenant.Ref.Slug, ActorEmail: ac.email, ActorRole: tenant.Role, Action: store.AuditActionVerwaltungSettings, TargetType: "organisation", TargetID: orgKey, Summary: "Verwaltungseinstellungen gespeichert", Details: map[string]string{"diff": summary}})
	}
	http.Redirect(w, r, "/app/verwaltung/einstellungen?flash="+url.QueryEscape("Einstellungen gespeichert"), http.StatusSeeOther)
}

func (a *app) renderVerwaltungSettings(w http.ResponseWriter, r *http.Request, ac authCtx, settings store.OrgSettings, flash string) {
	total := settings.Counters.Approved + settings.Counters.Edited
	share := 0
	if total > 0 {
		share = settings.Counters.Approved * 100 / total
	}
	data := web.VerwaltungSettingsData{Threshold: int(settings.AutoThreshold*100 + 0.5), AutoEnabled: settings.AutoEnabled, ProviderLabel: a.inboxProviderLabel(), Approved: settings.Counters.Approved, Edited: settings.Counters.Edited, Rejected: settings.Counters.Rejected, Auto: settings.Counters.Auto, UnchangedShare: share, Flash: flash}
	for _, category := range store.IntakeCategories() {
