...
func (s *SQLAnnualStatementRunStore) load(tx annualStatementRunQueryer, tenant TenantRef, year int, vectors map[string]AnnualStatementConsumptionVector) (AnnualStatementRunInput, error) {
	...
	err = read(`SELECT id,document_id,period_year,cost_type_key,amount_cents,invoice_date,created_at,created_by,updated_at,updated_by,supplier,heating_category FROM annual_statement_receipts WHERE tenant_id=$1 AND period_year=$2 ORDER BY id`, func(rows *sql.Rows) error {
