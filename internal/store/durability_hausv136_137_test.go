...
func TestAuditStoreToleratesTornTrailingLine(t *testing.T) {
	...
	good := `{"at":"2026-07-13T10:00:00Z","tenant":"jhw22","actor_email":"a@b.c","action":"login","summary":"ok"}` + "\n"
	torn := `{"at":"2026-07-13T10:01:00Z","tenant":"jhw22","act` // no newline, truncated
	...
	got := s.List(AuditFilter{TenantSlug: "jhw22", Limit: 10})
	if len(got) != 1 {
...
func TestAuditStoreRejectsCorruptMiddleLine(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "audit.jsonl")
	body := `{"at":"2026-07-13T10:00:00Z","tenant":"jhw22","actor_email":"a@b.c","action":"login","summary":"ok"}` + "\n" +
		`GARBAGE` + "\n" +
		`{"at":"2026-07-13T10:02:00Z","tenant":"jhw22","actor_email":"a@b.c","action":"login","summary":"ok2"}` + "\n"
