...
CREATE TABLE IF NOT EXISTS telegram_links (
    ...
    name text NOT NULL DEFAULT '', linked_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS contacts (
    ...
    data text NOT NULL DEFAULT '{}', PRIMARY KEY (tenant_id, id)
);
CREATE TABLE IF NOT EXISTS announcement_reads (
    tenant_id varchar(26) NOT NULL REFERENCES tenant(tenant_id) ON UPDATE RESTRICT ON DELETE CASCADE,
    email text NOT NULL DEFAULT '', seen_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, email)
);
CREATE TABLE IF NOT EXISTS announcements (
    tenant_id varchar(26) NOT NULL REFERENCES tenant(tenant_id) ON UPDATE RESTRICT ON DELETE CASCADE,
    id text NOT NULL DEFAULT '', data text NOT NULL DEFAULT '{}', PRIMARY KEY (tenant_id, id)
);
...
CREATE INDEX IF NOT EXISTS idx_house_memberships_tenant ON house_memberships(tenant_id);
CREATE TABLE IF NOT EXISTS integration_imports (
    ...
    applied_at timestamptz NOT NULL DEFAULT now(), applied_by text NOT NULL DEFAULT '',
    ...
    PRIMARY KEY (tenant_id, format, file_digest)
);
...
CREATE TABLE IF NOT EXISTS home_profiles (
    ...
    free_started_at timestamptz, free_until_at timestamptz,
    created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, home_key)
);
CREATE TABLE IF NOT EXISTS energy_assets (
    ...
    FOREIGN KEY (tenant_id, home_key) REFERENCES home_profiles(tenant_id, home_key) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_assets_tenant_home_kind ON energy_assets(tenant_id,home_key,kind,name);
CREATE TABLE IF NOT EXISTS energy_entity_mappings (
    ...
    updated_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id, id),
    UNIQUE (tenant_id,home_key,entity_id),
    FOREIGN KEY (tenant_id,home_key) REFERENCES home_profiles(tenant_id,home_key) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_entity_mappings_tenant_home_metric ON energy_entity_mappings(tenant_id,home_key,metric,confirmed);
CREATE INDEX IF NOT EXISTS energy_entity_mappings_tenant_home_asset ON energy_entity_mappings(tenant_id,home_key,asset_id,confirmed);
CREATE TABLE IF NOT EXISTS energy_intervals (
    tenant_id varchar(26) NOT NULL REFERENCES tenant(tenant_id) ON UPDATE RESTRICT ON DELETE CASCADE,
    home_key text NOT NULL DEFAULT 'default', starts_at timestamptz NOT NULL DEFAULT now(),
    ...
    created_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id,home_key,starts_at,source),
    FOREIGN KEY (tenant_id,home_key) REFERENCES home_profiles(tenant_id,home_key) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_intervals_tenant_home_time ON energy_intervals(tenant_id,home_key,starts_at DESC);
CREATE TABLE IF NOT EXISTS energy_imports (
    ...
    imported_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id,id), UNIQUE (tenant_id,home_key,sha256),
    FOREIGN KEY (tenant_id,home_key) REFERENCES home_profiles(tenant_id,home_key) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_imports_tenant_home_time ON energy_imports(tenant_id,home_key,imported_at DESC);
CREATE TABLE IF NOT EXISTS energy_maintenance_plans (
    ...
    last_completed_at timestamptz, next_due_at timestamptz NOT NULL DEFAULT now(), contact_id text NOT NULL DEFAULT '',
    document_id text NOT NULL DEFAULT '', issue_id text NOT NULL DEFAULT '', evidence_note text NOT NULL DEFAULT '',
    active boolean NOT NULL DEFAULT true, created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id,id), UNIQUE (tenant_id,home_key,asset_id),
    FOREIGN KEY (tenant_id,home_key) REFERENCES home_profiles(tenant_id,home_key) ON DELETE CASCADE,
    FOREIGN KEY (tenant_id,asset_id) REFERENCES energy_assets(tenant_id,id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_maintenance_tenant_home_due ON energy_maintenance_plans(tenant_id,home_key,active,next_due_at);
CREATE TABLE IF NOT EXISTS energy_tariff_assessments (
    ...
    data_quality text NOT NULL DEFAULT '', created_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id,id),
    FOREIGN KEY (tenant_id,home_key) REFERENCES home_profiles(tenant_id,home_key) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_tariff_assessments_tenant_home_time ON energy_tariff_assessments(tenant_id,home_key,created_at DESC);
CREATE TABLE IF NOT EXISTS energy_measures (
    ...
    appointment_at timestamptz, work_note text NOT NULL DEFAULT '', completed_at timestamptz,
    evidence_note text NOT NULL DEFAULT '', before_from timestamptz, before_to timestamptz,
    after_from timestamptz, after_to timestamptz, before_peak_kw double precision, after_peak_kw double precision,
    before_quality text NOT NULL DEFAULT '', after_quality text NOT NULL DEFAULT '',
    created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id,id), UNIQUE (tenant_id,home_key,issue_id),
    FOREIGN KEY (tenant_id,home_key) REFERENCES home_profiles(tenant_id,home_key) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS energy_measures_tenant_home_time ON energy_measures(tenant_id,home_key,updated_at DESC);
...
CREATE TABLE IF NOT EXISTS home_reservations (
    ...
    created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
    confirmed_at timestamptz, PRIMARY KEY (tenant_id)
);
...
CREATE TABLE IF NOT EXISTS home_connectors (
    ...
    pairing_hash bytea, pairing_expires_at timestamptz, connector_version text NOT NULL DEFAULT '',
    ha_version text NOT NULL DEFAULT '', entity_count integer NOT NULL DEFAULT 0,
    created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
    paired_at timestamptz, last_seen_at timestamptz, PRIMARY KEY (tenant_id),
    FOREIGN KEY (tenant_id) REFERENCES home_reservations(tenant_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS home_connector_readings (
    ...
    last_updated timestamptz NOT NULL DEFAULT now(), received_at timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id,entity_id),
    FOREIGN KEY (tenant_id) REFERENCES home_connectors(tenant_id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_home_connector_readings_tenant_received ON home_connector_readings(tenant_id,received_at DESC);
